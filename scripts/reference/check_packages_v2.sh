...
# Cache available packages for faster lookup
echo "🔍 Building package cache..."
PACKAGE_CACHE=$(mktemp)
trap "rm -f $PACKAGE_CACHE" EXIT

# Get all available packages (both python3-* and general packages)
{
    apt-cache --no-generate pkgnames python3- 2>/dev/null || true
    apt-cache --no-generate pkgnames 2>/dev/null | grep -E "^(scons|black|flake8|mypy|pytest|tox|cookiecutter|jupyter|ipython|notebook|jupyterlab|streamlit|pre-commit|poetry|pipenv|twine|bandit|ansible|awscli|httpie|youtube-dl|yt-dlp|inflect)$" || true
} | sort | uniq > "$PACKAGE_CACHE" || {
    echo "⚠️  Falling back to dpkg-query..."
    {
        dpkg-query -l 'python3-*' 2>/dev/null | awk '/^ii/ {print $2}' || true
        dpkg-query -l scons black flake8 mypy pytest tox cookiecutter jupyter ipython notebook jupyterlab streamlit ansible awscli httpie 2>/dev/null | awk '/^ii/ {print $2}' || true
    } | sort | uniq > "$PACKAGE_CACHE"
}

echo "Found $(wc -l < "$PACKAGE_CACHE") available packages (python3-* + common CLI tools)"
...
is_available_in_apt() {
    ...
    local variations=(
        ...
        "$package"                          # direct package name (for non-python packages like scons)
    )
...
check_package() {
    ...
    # PRIORITY 1: Check if available in apt (always check this first!)
    if apt_package=$(is_available_in_apt "$package"); then
        echo "$package:$apt_package" >> "$temp_dir/apt"
        return 0
    fi
    
    # PRIORITY 2: Only suggest pipx if NOT available via apt
    if [[ -n "${KNOWN_CLI[$package]}" ]]; then
        echo "$package" >> "$temp_dir/pipx"
        return 0
    fi
    
    # PRIORITY 3: CLI heuristics for unknown packages (also only if not in apt)
...
if [[ ${#pipx_candidates[@]} -gt 0 ]]; then
    echo -e "\n${BLUE}🔧 APPLICATIONS (pipx - not available via apt):${NC}"
    printf 'pipx install %s\n' "${pipx_candidates[@]}"
    echo -e "${YELLOW}💡 These CLI tools are not available as system packages${NC}"
fi
